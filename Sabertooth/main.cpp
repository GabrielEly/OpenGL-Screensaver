...
// Adaptado de código do professor Leandro Tonietto
// Projeto de Screensaver
...
int main() {
	...
	*/
	...
	window = glfwCreateWindow(640, 480, "Screensaver", NULL, NULL);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (elapsedSeconds > 0) {
			...
			if (fabs(lastPositionX) > 0.75f) {
				xspeed = -xspeed;
			}
			else if (fabs(lastPositionY) > 0.75f) {
				yspeed = -yspeed;
			}
			matrix[12] = elapsedSeconds * xspeed + lastPositionX;
			matrix[13] = elapsedSeconds * yspeed + lastPositionY;
